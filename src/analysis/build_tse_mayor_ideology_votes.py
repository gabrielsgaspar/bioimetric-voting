...
PARTY_VOTES_PATH = INTERIM_DIR / "tse_mayor_party_votes_bdd_2000_2020.parquet"
PARTY_VOTES_CSV_PATH = INTERIM_DIR / "tse_mayor_party_votes_bdd_2000_2020.csv"
OUTPUT_PATH = CLEAN_DIR / "tse_mayor_ideology_votes.parquet"
OUTPUT_CSV_PATH = CLEAN_DIR / "tse_mayor_ideology_votes.csv"
DIAGNOSTICS_PATH = INTERIM_DIR / "tse_mayor_ideology_votes_diagnostics.csv"
QUERY_LOG_PATH = INTERIM_DIR / "tse_mayor_ideology_votes_bigquery_log.csv"
NOTES_PATH = DOCS_DIR / "TSE_MAYOR_IDEOLOGY_VOTES_NOTES.md"
...
def mayor_party_votes_query() -> str:
...
def build_ideology_shares(party_votes: pd.DataFrame, ideology: pd.DataFrame) -> pd.DataFrame:
    ...
    votes["mayor_votes"] = pd.to_numeric(votes["mayor_votes"], errors="coerce").fillna(0)
    votes = votes.merge(ideology, on="party_key", how="left")
    votes["ideology_lcr"] = votes["ideology_lcr"].fillna("IDK")

    grouped = (
        votes.groupby(["year", "state", "municipality_id", "ideology_lcr"], as_index=False)["mayor_votes"]
        .sum()
    )
    totals = (
        grouped.groupby(["year", "state", "municipality_id"], as_index=False)["mayor_votes"]
        .sum()
        .rename(columns={"mayor_votes": "total_mayor_votes"})
    )
    grouped = grouped.merge(totals, on=["year", "state", "municipality_id"], how="left")
    grouped["vote_share"] = grouped["mayor_votes"] / grouped["total_mayor_votes"]
    ...
    out_cols = ["year", "state", "municipality_id", "pct_mayor_R", "pct_mayor_C", "pct_mayor_L", "pct_mayor_IDK"]
    out = wide[out_cols].sort_values(["year", "state", "municipality_id"]).reset_index(drop=True)
    for col in ["pct_mayor_R", "pct_mayor_C", "pct_mayor_L", "pct_mayor_IDK"]:
        out[col] = out[col].astype(float)
    return out


def build_diagnostics(out: pd.DataFrame, party_votes: pd.DataFrame) -> pd.DataFrame:
    share_cols = ["pct_mayor_R", "pct_mayor_C", "pct_mayor_L", "pct_mayor_IDK"]
...
def main() -> None:
    ...
    party_votes = query_to_frame(
        client,
        "bdd_tse_mayor_party_votes_2000_2020",
        mayor_party_votes_query(),
        PARTY_VOTES_PATH,
        force=args.force,
        query_log=query_log,
    )
