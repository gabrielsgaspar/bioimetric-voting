...
OUTPUT_DIR = ROOT / "resources" / "regressions" / "tse_mayor_ideology_bvr2016"
RESULTS_PATH = OUTPUT_DIR / "bvr2016_mayor_ideology_regressions.csv"
SAMPLE_PATH = OUTPUT_DIR / "bvr2016_mayor_ideology_sample.csv"
DIAGNOSTICS_PATH = OUTPUT_DIR / "bvr2016_mayor_ideology_diagnostics.csv"
...
def build_sample() -> pd.DataFrame:
    ...
    status = pd.DataFrame(
        {
            "municipality_id": sorted(treated_ids | control_ids),
        }
    )
    ...
    sample = mayor.merge(status, on="municipality_id", how="inner")
        ...
        diagnostics.append(
            {
                ...
                "mean_pct_mayor_R_treated": group.loc[group["treated_bvr_2016"].eq(1), "pct_mayor_R"].mean(),
                "mean_pct_mayor_R_control": group.loc[group["treated_bvr_2016"].eq(0), "pct_mayor_R"].mean(),
                "mean_pct_mayor_L_treated": group.loc[group["treated_bvr_2016"].eq(1), "pct_mayor_L"].mean(),
                "mean_pct_mayor_L_control": group.loc[group["treated_bvr_2016"].eq(0), "pct_mayor_L"].mean(),
                "mean_pct_mayor_C_treated": group.loc[group["treated_bvr_2016"].eq(1), "pct_mayor_C"].mean(),
                "mean_pct_mayor_C_control": group.loc[group["treated_bvr_2016"].eq(0), "pct_mayor_C"].mean(),
            }
        )
